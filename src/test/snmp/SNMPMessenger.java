...
public class SNMPMessenger {
    ...
    // public void getTable(OID[] columns, final SNMPTableResponseListener
    // responseListener) {
    // try {
            ...
            ResponseListener listener = new ResponseListener() {
                public void onResponse(ResponseEvent event) {
                    ...
                    if (response != null) {
                        ...
                        if (errorStatus == PDU.noError) {
                            System.out.println("Snmp Set Response = "
                                    + response.getVariableBindings());
                            responseListener.onSNMPResponseReceived(response.getVariableBindings());
                        } else {
                            System.out.println("Error: Request Failed");
    ...
    // public static void discoverDevices(int[] begin, int[] end, final
    // ManagerFrame frame) {
    // int[] current = begin.clone();
    // SNMPMessenger messenger = new SNMPMessenger(current[0] + "." + current[1]
    // + "." + current[2] + "." + current[3],
    // ManagedDevice.port);
    //
    // for(int i = begin[3]; i <= end[3]; i++) {
    // current[3] = i;
    // final String currentIP = current[0] + "." + current[1] + "." + current[2]
    // + "." + current[3];
    //
    // messenger.setIP(current[0] + "." + current[1] + "." + current[2] + "." +
    // current[3]);
    //
    // messenger.sendGetRequest(new OID(new int[] {1,3,6,1,4,1,12619,1,1,1,0}),
    // new SNMPResponseListener() {
    // public void onSNMPResponseReceived(Vector<? extends VariableBinding>
    // variableBinding) {
